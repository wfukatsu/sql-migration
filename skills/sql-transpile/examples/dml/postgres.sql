-- sql-transpile のテスト用 SQL: DML と実用的な SELECT のパターン（変換元: PostgreSQL）
...
-- 実行検証: .venv/bin/python difftest/transpile_verify.py --source postgres --examples-dir skills/sql-transpile/examples/dml
...
CREATE TABLE audit_log (log_id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 100) PRIMARY KEY, table_name VARCHAR(30), action VARCHAR(10), logged_at TIMESTAMP);
...
-- @expect-scalardb: OK
INSERT INTO customers (customer_id, name, email, region, vip, created_at) VALUES (7, 'Grace Lee', 'grace@example.com', 'SOUTH', FALSE, DATE '2024-09-01');
-- @id: I02
...
-- @expect-scalardb: ERROR
INSERT INTO audit_log (log_id, table_name, action, logged_at) VALUES (3, 'customers', 'LOGIN', CURRENT_TIMESTAMP);
-- @id: I07
...
-- @expect-scalardb: ERROR
INSERT INTO orders (order_id, customer_id, order_date, status, total, note) VALUES (nextval('order_seq'), 3, DATE '2024-09-03', 'NEW', 5.25, NULL);
-- @id: I09
-- @note: あれば加算、無ければ挿入する upsert（列を参照する更新）
-- @check: SELECT qty FROM stock WHERE warehouse_id = 1 AND product_id = 102
-- @expect-scalardb: ERROR
INSERT INTO stock (warehouse_id, product_id, qty) VALUES (1, 102, 10) ON CONFLICT (warehouse_id, product_id) DO UPDATE SET qty = stock.qty + EXCLUDED.qty;
-- @id: I10
...
-- @expect-scalardb: WARN
INSERT INTO products (product_id, name, category, price, active) VALUES (102, 'Mouse Pro', 'PERIPHERAL', 24.00, TRUE) ON CONFLICT (product_id) DO UPDATE SET name = EXCLUDED.name, price = EXCLUDED.price;
-- @id: I11
...
-- @expect-scalardb: ERROR
INSERT INTO customers (customer_id, name, email, region, vip, created_at) VALUES (1, 'Alice Duplicate', NULL, 'EAST', FALSE, DATE '2024-09-04') ON CONFLICT (customer_id) DO NOTHING;
-- @id: I12
...
-- @expect-scalardb: ERROR
UPDATE orders o SET note = 'vip' FROM customers c WHERE c.customer_id = o.customer_id AND c.vip = TRUE;
-- @id: U08
...
-- @expect-scalardb: ERROR
UPDATE orders SET order_date = order_date + INTERVAL '7 days' WHERE order_id = 1005;
-- @id: U11
...
-- @expect-scalardb: ERROR
UPDATE orders SET status = 'HOLD' WHERE order_id = (SELECT order_id FROM orders WHERE status = 'NEW' ORDER BY order_date, order_id LIMIT 1);
-- @id: U12
...
-- @expect-scalardb: ERROR
DELETE FROM order_items i USING orders o WHERE o.order_id = i.order_id AND o.status = 'CANCELLED';
-- @id: D08
...
-- @expect-scalardb: ERROR
DELETE FROM audit_log WHERE log_id IN (SELECT log_id FROM audit_log ORDER BY logged_at, log_id LIMIT 1);
-- @id: D09
...
-- @expect-scalardb: ERROR
DELETE FROM orders WHERE order_id = 1003 RETURNING order_id, status;
-- @id: S01
...
-- @expect-scalardb: WARN
SELECT customer_id, name FROM customers WHERE name ILIKE 'frank%' ORDER BY customer_id;
-- @id: S11
-- @note: 月ごとに丸めて集計（Oracle は TRUNC、PostgreSQL は DATE_TRUNC、MySQL は DATE_FORMAT）
-- @expect-scalardb: PLANNED
SELECT DATE_TRUNC('month', order_date) AS month, COUNT(*) AS n, SUM(total) AS amount FROM orders GROUP BY DATE_TRUNC('month', order_date) ORDER BY month;
-- @id: S12
...
-- @expect-scalardb: PLANNED
SELECT customer_id, name FROM customers WHERE vip ORDER BY customer_id;
-- @id: S17
