-- sql-transpile のテスト用 SQL: DML と実用的な SELECT のパターン（変換元: MySQL）
...
-- 実行検証: .venv/bin/python difftest/transpile_verify.py --source mysql --examples-dir skills/sql-transpile/examples/dml
...
-- @id: S02
-- @note: キーセットページング（行値の比較。Oracle は OR で書く）
...
-- @id: S12
-- @note: NULL の並び位置を指定（MySQL には NULLS LAST が無いので IS NULL で並べる）
-- @expect-scalardb: PLANNED
