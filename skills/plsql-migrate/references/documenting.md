...
Oracle 自身が結果を決めていないシナリオ（`ROWNUM <= n` に `ORDER BY` が無い更新など）は、`nondeterministic` に理由と比べない列（`ignore_columns: {表: [列, …]}`）を書く。その列を両側から外した行の集合で比べるので、何件がどう変わったかは確かめ続ける。比較の報告には「非決定として比べない列」として理由つきで出る。理由の無い宣言は拒否される
