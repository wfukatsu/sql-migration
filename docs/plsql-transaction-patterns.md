...
**再試行の責務がどこに来るかを決めること。** それがこのページのすべての型に共通する宿題である。
...
**決めること**: 最大再試行回数、待ち時間、そして**再試行してよい操作か**。冪等でない副作用
（採番、外部通知）が同じトランザクションにあれば、再試行は二重実行になる。
...
1. **番号に意味があるか。** 連番でなくてよいなら UUID（ただし列型が変わる）。
2. **欠番を許すか。** 許すなら範囲を先に確保して配る（hi/lo）。衝突が激減する。
3. **順序を保つか。** 厳密な単調増加が要るなら、この counters 方式 + 再試行になる。**その場合、
   採番は高衝突点になる**ので、他の更新と同じトランザクションに入れない。
