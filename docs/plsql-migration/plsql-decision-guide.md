...
**書いた表を同じトランザクションで走査する（SCAN-001）** のも、境界の問題である。ScalarDB は、同じトランザクションで
書いた行を含む走査を断る。直し方は、読みを書き込みの前に移す（結果が書き込みに依存しないと確かめたうえで）か、読みだけを
別の読み取り専用トランザクションで行うかで、どちらもアプリの設計の判断である。生成器は断ったままにする
（samples/oracle-samples b06_3 は、利用者の決定で再設計として記録した。2026-09-26）。FORALL の反復ごとに読んでから書く形は、
反復ごとに読む範囲が重ならなければ実 DB で通るが、同じキーが 2 回来ると断られる。静的解析は警告を残す。
...
**選び方。** 読み取りだけなら計画に回す。`ROWNUM` に `ORDER BY` が無い・同順位の並びが決まらない・時計や接続ユーザを
返す文は、比較のケースで理由つきに宣言し（`-- @nondeterministic: count | unordered | ignore=列; reason=…`、#57）、
緩めた比べ方で比べる。移行元が受け付けない文は `-- @source-rejects: sample | harness; reason=…`（#58）。
更新を含む文はアプリの設計。
...
scanRows:
  routines:
    b04_4_1_explicit_cursor: 1000     # 1 部門の社員。1000 人を超える部門は業務側で確かめる異常（2026-09-26）
    b06_1_bulk_collect_limit: 100000  # 全社員。数列の行なので数十 MB（2026-09-26）
rowLocks:
  optimistic:
