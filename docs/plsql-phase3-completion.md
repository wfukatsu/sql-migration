...
| KPI-6 人手修正時間 | ベースライン確定 | 未計測（0/56 routine） | 当時**未達**。→ 2026-09-17 に計測しないと決定（計画 §9） |
| KPI-7 未解決リスク密度 | 目標なし（トレンド） | 25.8 件/1000 行（24 ERROR / 931 行） | — |

**KPI-6 はこの報告の時点で未達だった。** 誰も実作業時間を測っていないので `decisions.json` は `null` の
ままであり、「REVIEW 1 routine あたりの中央値をベースラインとして確定する」という目標は達成していない。
枠組みは用意したが、値は人が REVIEW を実際に消化しないと出ない。**数字を作って埋めることはしていない。**

> **追記（2026-09-17）**: その後、**KPI-6 を本 PoC では計測しないことを決めた**（計画 §9）。したがって
> これは未達ではなくなったが、**測っていないという事実は変わらない**——本 PoC は移行工数を測っていない。
