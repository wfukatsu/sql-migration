...
```mermaid
flowchart TD
    A["SQL スクリプト"] --> B["文に分割<br/>Oracle: / だけの行は切れ目<br/>PL/SQL ブロックは 1 つにまとめる"]
    B -- "PL/SQL ブロック" --> PB["ERROR PLSQL_BLOCK"]
...
| 約束 | 中身 |
|---|---|
| 文の分割 | 方言の tokenizer でトップレベルの `;` を探す（コメントと文字列の中は切らない）。Oracle の入力は SQL*Plus のスクリプトとして読み、`/` だけの行を切れ目にする。PL/SQL のブロックは `/` までを 1 つにまとめ、どちらの経路でも ERROR `PLSQL_BLOCK` にする（中の `;` で切ると、`END` が式として OK になり、後ろの文が失われた）。式として解析された断片は ERROR `PARSE` |
| 終了コード | 0 = ERROR の文なし / 1 = ERROR の文あり / 2 = 入力の誤り。2 のときレポートは書かない（変換する文が無い、形の違う `--schema`、不正な `--session-time-zone` も 2）。BOM は読み飛ばす |
| 出力 | レポートの名前に Target は入らない（Target ごとに `--out-dir` を分ける）。`--plan-dir` は、その入力の古い計画を消してから書く。レポートの表は、SQL の中のバッククォートで崩れない |
| 指摘コード | 意味は `references/` の表が正。変換器が出すコードがすべて載っていることをテストが確かめる |
...
| 承認の記録 | 承認した人（役割）・日付・**承認したときのファイルの指紋**（SHA-256）・対象のファイル。未決の判断（記録の未決の項目、判定が REVIEW の routine、`limits.yaml` に答えの無いルールが残る REDESIGN の routine）を残して進めるなら、利用者が決めた理由も控える |
| 指紋の対象 | `spec` = `spec/*.md` と**原文**（PL/SQL のディレクトリ、または SQL のファイル）。`decisions` = `limits.yaml`、記録、変換の報告。`converted` = `docs/*.md`。入力のパスは `init` のときに絶対パスで控える（どこから呼んでも同じ指紋になる） |
| REDESIGN の扱い | 判定は決定のあとも REDESIGN のまま動かさないので、決まったかどうかは決定を適用した解析（`<out>/generated/analysis/decisions.json` の `redesign.open`）から読む。解析が無いか `limits.yaml` より古ければ「決めたかどうかが分からない」として未決に挙げる。このため、その解析は段階 2 で作る |
| SQL の ERROR の文 | `--kind sql` では、変換できなかった文の 1 つずつに `SQL-<文の番号>` の項目が記録に無ければ `decisions` を承認に出せない（記録のパスを渡してあるだけでは通らない） |
| 承認の失効 | 指紋が合わなくなった承認は「古い」になり、`gate` が閉じる。`spec` か `decisions` を取り直すと、前のテストの結果は消える（テストが確かめたのは、現行の仕様と、決定を適用して生成したコードだから）。`converted` は文書なので、取り直してもテストの結果は残る |
| テストの結果と文書 | `tested --report` は比較のファイルを `inputs.evidence` に控える。比較の結果を文書に入れる（`migration_doc.py --evidence`）と `converted` は古くなるが、検査が同じ比較を見るので取り直せる。取り直した日はテストの結果に残る |
| 入力の誤り | 形の違う記録、壊れた JSON / YAML、無いファイルは終了コード 2（1 は「まだ」のためにある） |
...
- **事実の欄**（`<!-- facts:begin ID -->` 〜 `<!-- facts:end ID -->`）は機械的に出す。作り直しても、欄の外の文章には触れない。
  文は IF / ELSIF / CASE の分岐、ループ、例外ハンドラの中まで辿り、そこへ至る条件（`WHEN p_mode = 'A'`）を添える。
  処理の流れの図は、ELSE の無い CASE に `CASE_NOT_FOUND` の出口を、`EXIT` / `CONTINUE` にループを抜ける線・先頭へ戻る線を
  描く。読み書きする表は `plsql.cli` が SQL そのものから埋めてから IR を書き出すので、ScalarDB の schema を渡さない
  解析（現行の仕様を調べるときの形）でも入る
