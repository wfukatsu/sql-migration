...
**KPI-6（人手修正時間）は測定値であって推定値ではない**（本 PoC では計測しないと決めたが、仕組みは残してある。§9）。 `decisions.json` は routine ごとに枠を持ち、
...
**意図的な差 1 件（2026-09-26 に取りやめた）**: 除算の結果を Oracle クライアントは 40 桁返すことがあるが、NUMBER が
保証するのは 38 桁なので、比較は NUMBER の精度で行う、としていた。ところが中間値は保存されなくても**印字で見える**
（Oracle の言語リファレンスの例 11-21: `1/3` の DBMS_OUTPUT が 40 桁）。#64 で `OracleNumbers.divide` を Oracle の
格納形式（100 進 20 桁の仮数、先頭の組が 2 桁なら 40 桁・1 桁なら 39 桁）に合わせた。AVG も同じ割り算を通る。
...
**P4-10 `plsql/remediate.py`。** ルールが人へ回した routine には、ルール id と 1 文しか付いていない。
モデルはもっと言える——その構文が何をしていたか、記録済みの型のどれに当たるか、移行後の形はどうなるか。
有用であり、同時に**このパイプライン全体で最も害をなしやすい場所**なので、3 つを助言ではなく構造にした。

1. **既定では何も外へ出ない。** リクエストを作るのはファイルを書くだけで、送るのは別の手順である。
   顧客の PL/SQL を第三者へ渡してよいかは**顧客の判断**であって、ツールの既定ではない。
   `--prepare` は 43 件のリクエストと **`what-would-be-sent.md`**（何が出るか・何は出ないか）を書いて
   止まる。provider の既定は `unconfigured` で、**理由を述べて拒否する**。
2. **助言は証拠にならない。** 判定エンジンはこの出力を読まないし、読めない——助言は別ファイルに住み、
   自分の判定を持ち、それは常に REVIEW である。完璧なコードを出したモデルでも routine を AUTO には
   上げない。**Oracle と突き合わせていないから**で、それが AUTO の意味である（KPI 定義 §3）。
   モデルの確信は測定の代わりにならない。エンジンのソースに `remediate` の文字が無いことをテストで固定した。
3. **どの答えにも出所が付く。** モデル・時刻・リクエストの digest。リクエストが変わった助言は
   **stale** として報告し、現在のものとして見せない。

**P4-11 `plsql/propose.py`。** 人が解決した REVIEW ごとに、ルールが下せなかった判断が 1 つある。
同じ形に同じ判断が繰り返されたら、それは書かれるのを待っているルールである。

**提案はルールではなく、事故でルールになることもない。** 提案はルールディレクトリの**外**に、
ルールとして読み込めない形で書かれる。`RuleSet.load` は `plsql/rules/*.yaml` しか読まないので、
人がそこへ書き写して初めて有効になる——**それが要点である**。ツールが自分の提案を自分で採用すれば、
判定を人がレビューできるようにしている仕組みそのものが意味を失う。

束ねるのは**判定を決めたルール**であって、当たった全ルールではない。付随的な一致（どこかの `SYSDATE`、
ScalarDB が実行できない文）を含めると routine ごとに固有の形になり、何も提案されない。無視した
ルールは提案に併記する。下限は 3 件——**2 件は偶然である**。
...
採番方式の決定（§9）を実装した。**どちらに寄せるかを人に書かせず、移行元の DDL から導く。**
...
持ち込む性質そのもの**である。`NOCACHE` はその逆で、欠番を避けたいという意思表示である。だから既定は
DDL から導き、**人が書くのは例外だけ**——設定を一から書かせると、元が何を言っていたかを読まずに埋める
ことになる。上書きには**理由を要求する**（DDL を覆すのだから）。

corpus の 4 つは宣言どおりに分かれた: `seq_audit_id` / `seq_tx_id` → hi/lo（block 100）、
`seq_order_id` / `seq_payment_id` → counters + 再試行。`START WITH` も引き継ぐ——
**1 から始め直すのは移行ではなく破壊である**。

| 層 | もの |
|---|---|
| 方式の導出 | `plsql/sequences.py` |
| 生成 | `seq_x.NEXTVAL` を P4-4 の式持ち上げに載せ、`sequences.next("seq_x")` に翻訳 |
| 実行時 | `Sequences`（インタフェース）と `CountersSequences`（既定実装） |

**インタフェースにしたのは、採番がどのトランザクションで走るかを移行先が決めるべきだから**である。
生成コードは境界を持たない（§9）ので、実装を固定すると生成器がその決定を奪う。既定実装は採番専用の
接続を受け取り、**業務トランザクションとは別のトランザクションで確保する**——同じにすると採番行が
高衝突点になって業務の更新まで巻き込む（P3-4 の測定）。採番する Repository にだけ依存を足す。

**判定は動かなかった。** `SEQUENCE` エラー 7 件は消えたが、**同じ文に別の障害が残っていた**——
`USER`（誰を記録するかは決定事項、trigger テンプレート §A）、`:NEW` / `:OLD`（trigger 由来）、
コレクション要素（BULK 由来）、行ロックのため持ち上げを抑止している式。採番はそれらの手前にあった
だけで、**採番が唯一の障害だった文は無かった**。

テストが実装の欠陥を 1 件見つけた。**行が無いという失敗まで 10 回再試行していた**——絶対に成功しない
...
| ~~P4-2~~ KPI-6 ベースライン | **取り下げ**（KPI-6 を計測しないと決定） |
...
| P4-9 引き渡し版 | 完了 |
| P4-10 LLM Remediator | 完了（既定では何も送らない。助言は常に REVIEW） |
| P4-11 ルール提案 | 完了（提案はルールにならない） |
...
**設計テンプレートは書いたが、決めるのは人である。**

**Phase 4 完了条件**: 非 AUTO の件数が半減し（56 → 28 以下）、残るものは設計テンプレートか、
「この routine は人が決めるべき」という説明のどちらかを持つ。
（KPI-6 のベースラインは完了条件から外した——2026-09-17 の決定で計測しないため。）
...
- **キーで届かない `SELECT INTO` も、Oracle の意味論を保つ**（2026-09-19 決定、#4）。条件に一致する行を最大 2 件
  ...
  - 判定（2026-09-20 決定）: SELECT-001 は「移行後も Oracle の意味論を再現できるか」と「データモデルに一意性を明示
    すべきか」を混ぜていた。前者は生成コードと実 DB の evidence で解決済みなので、REVIEW は**意味論の保存を保証
...
- **REDESIGN の判定は動かさず、再設計の状態を分けて見せる**（2026-09-20 決定）。REDESIGN は移行元の分類で、行ロック・
  ...
  - **DB Link は、link の先の表も ScalarDB の namespace として管理下に置き、同じトランザクションで書く**
    ...
    - 一致しないのは失敗時の**例外の種類** 1 点。Oracle は link 越しの失敗を ORA-02055 で返し、トランザクション
      ...
      （行が既にある）ので、呼び出し側は無条件の再試行にしないこと。
    - **この差は受け入れる**（2026-09-20 の決定）。シナリオに `accepted_difference`（Oracle と移行先の
      例外コードの組・理由・決定日）を書くと、比較はその組にだけ当たる例外コードの差を `differences` から
      `accepted` へ移す。隠すのではなく、報告に「受け入れた差」として理由つきで出す。別のコードが出たとき、
      表や戻り値が違ったときは、これまでどおり相違になる（`plsql_compare._accepted`）。
  ...
  - 2026-09-20 の corpus: 27 件すべてが決定済み・実 DB で一致、未決定 0（`prc_remote_sync` は上記の受け入れた差 1 点つき）。採番 trigger
...
- **PoC の合否基準を先に固定し、判定者は後で決める**（2026-09-17）。
  ...
  | KPI-6 人手修正時間 | **計測しない**（2026-09-17 の決定）。合否に数えない |
...
- **再生成モデルは引き渡し時点で終了する**（2026-09-17）。
  ...
  - **帰結 1**: 生成物のヘッダにある「編集するな。変更はルール・生成器側へ」は、**引き渡し後は誤り**に
    なる。引き渡し版では文言を変える（P4-9 `--handover` で実装済み）。
  ...
  - **引き渡し版へ切り替える時期は、日付ではなく条件で決める**（2026-09-20、#7）。
    **決まると生成コードが変わるものが残っていないこと**——REVIEW の routine が無く、未決定の REDESIGN が
    無いこと。切り替えた後はツールを再実行しない（手編集が失われる）ので、早すぎる切り替えは、ツールが一括で
    入れられた変更を、手編集済みのコードへ人が手で入れ直すことを意味する。生成コードを変えない決定
    （合否の判定者 #6、工数の計測 #17）は、引き渡しを待つ理由にならない。
    - 条件はツールが守る: `--handover` は条件を満たさないとき、残っているものを名指しして 1 を返し、
      **何も書かない**（引き渡し版の見出しが付いた tree は、誰かが手編集を始めてしまう）。承知のうえで
      進めるなら `--handover-anyway`。
    - 2026-09-20 の corpus: `--limits fixtures/plsql/limits.yaml` を渡せば条件を満たす（REVIEW 0、
      未決定の REDESIGN 0）。決定を渡さなければ 19 件が残り、拒否される。
    - 見送ったもの: routine 単位の引き渡し（先に引き渡した routine だけ再生成から外す）。条件を満たした今は
      要らない。実案件で決定が長く残る routine が出たら、そのとき実装する。

- **KPI-6（人手修正時間）は本 PoC では計測しない**（2026-09-17）。
  実作業時間は人が REVIEW を実際に消化しないと出ず、PoC の範囲でそれを行わない。目標値も置かない。
  - **帰結: 本 PoC は移行工数を測っていない。** KPI-6 だけが移行工数そのものを測るもので、他の 6 つは
    ツール内部の健全性指標である。**AUTO 率が上がっても REVIEW の手戻りが重ければ移行は速くならない**——
    その重さは本 PoC の数値からは分からない。工数の見積りが要る段階になったら計測する。
  - 仕組みは残す。`decisions.json` の枠、`--fix-times`、`plsql/propose.py` の resolutions はそのままで、
    **使える状態のまま使わないことを決めた**だけである。
  - あわせて **P4-2 を取り下げる**。Phase 4 の完了条件からも外す。

- **設計判断 4 件を決めた**（2026-09-17）。非 AUTO を塞いでいた「人が決めること」のうち、
  影響範囲の大きい 4 つである。テンプレート（`docs/plsql-*-patterns.md`）は各決定に沿って読むこと。

  1. **採番は用途ごとに使い分ける。** 監査 ID など代理キーは **hi/lo**（範囲を先に確保して配る。欠番は
     出るが単調増加で、衝突が激減する）。業務上番号に意味があるもの（注文番号・支払番号）は
  ...
  2. **走査行数の上限は routine ごとに指定し、既定を config に置く。**
     ...
     超えたら**例外で止まる**——OOM で落ちるより、限度を超えたと言って止まる方が良い。
     - 帰結: **既定値そのものに根拠は無い。** 各 routine について「何行まで来うるか」を業務側と決めて
       `limits.yaml` に書き、根拠をコメントに残すまで、既定は「決めていない」という意味である。

  3. **衝突時の再試行は use case 境界が持つ。** トランザクションを開始・commit する層が再試行も持つ。
     生成コードが境界を持たないという既定（§9）と整合し、**再試行してよい操作かを判断できる唯一の層**
     でもある（冪等でない副作用が同じトランザクションにあれば、再試行は二重実行になる）。
     - 帰結: Repository は自動再試行しない。業務例外（`MigratedException`）と衝突
       （`SQLTransactionRollbackException`）を取り違えないこと——再試行して結果が変わるのは後者だけである。

  4. **時刻は UTC に固定し、表示層で変換する。** `Plsql.systimestamp()` は JVM の既定 TZ ではなく
     **UTC を返す**ようにした。既定 TZ のままだと、同じコードが動く場所によって別の瞬間を書くことになり、
     Oracle との比較が機械の都合で通ったり落ちたりする。
     - 帰結: **既存データの移行時に TZ 変換が要る。** 帳票の日付境界（日次締めなど）がずれる可能性を
       確かめること。`SEM-002` は当面 REVIEW のまま残す——決定は変動を無くしたが、
       **UTC 固定後の挙動を Oracle と突き合わせた証拠はまだ無い**。
