...
- 実 DB: 7 シナリオ中 6 が Oracle と一致（表の状態・OUT 引数・例外コード）。
  ...
  - 一致しないもの: `create_order_duplicate_id`。採番した番号の受注が既にあるとき、Oracle は `DUP_VAL_ON_INDEX` の
    handler が -20004 に変える。ScalarDB は重複 INSERT をその文では弾かず、commit 時の衝突（`DB-CORE-20013`）として
    返すので、handler は走らない。どちらも何も書かない。解析が EXC-001 で先に指摘していたとおりの差で、
    受け入れるか（採番が一意なら起きない）、INSERT の前に読む形へ書き直すかは人が決める。
