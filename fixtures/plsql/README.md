...
**2026-09-20**: 判定の方針が変わり（REVIEW は移行の可否が未解決のときだけ）、holdout2 の `pkg_shipment.is_shippable`・
`line_count`・`days_in_transit` は期待値（REVIEW）と判定（AUTO）が食い違っている。期待値は書き換えず、食い違いとして
報告する（KPI-3 は 64/67）。

`src/holdout/` と `src/holdout2/` の 9 ユニット（14 ファイル、全 29 ユニット中 31.0%）は、
...
そのファイルは holdout ではなくなるので、この README に記録する。

### holdout の独立性は P2-2 / P2-4 で失われた（2026-09-17）

ルールエンジン（P2-2）の開発中、判定適合率を上げる過程で期待値との差分一覧を繰り返し出力した。
その一覧には holdout の routine 名と期待判定が含まれており、**どのルールを足すかの判断にそれを使った**。
`pkg_payment` / `pkg_order_lock` / `pkg_customer_import` / `prc_reprice_all` が該当する。

したがって **現時点の holdout 上の判定適合率は、独立した証拠ではない**。数字としては報告するが、
「ルール作成時に参照していないデータでの成績」としては読めない。

これを回復するため、**`src/holdout2/`** を足した（2026-09-17）。

- ルールが固まった**後**に書き起こした 4 ユニット / 8 routine
- 期待判定は**エンジンを走らせる前**に manifest へ確定させた
- 結果が悪くてもルールを直さず、数字をそのまま報告する

この取り決めのもとでの判定適合率は **8/8 = 100%** だった（P2-4 時点）。
`holdout/` の 10 routine は引き続き「参照済み」として扱い、独立した証拠には数えない。

今後 `holdout2/` を見てルールを直した場合は、同じようにここへ記録し、次の独立ホールドアウトを作る。
実案件由来のコードが入手できれば、それが最も強い holdout になる。
