...
script_unit
    : plsql_unit {self.setLastUnitPlsql();}
    | sql_unit {self.setLastUnitSql();}
    | sql_plus_command {self.setLastUnitSql();}
    ;
...
terminator
    : SEMICOLON
    | SEMICOLON {self.isSolidusSeparator()}? SOLIDUS
    | {self.isLastUnitSql()  and  self.isSolidusSeparator()}? SOLIDUS
    ;
...
table_ref_aux
    : table_ref_aux_internal flashback_query_clause* ({self.isNotStartOfJoin()}? table_alias)?
    ;
...
dml_table_expression_clause
    ...
    | {self.isVersion11()}? (RELATIONAL '(' tableview_name NOT XMLTYPE ')')
    ;
...
unary_expression_core
    ...
    | standard_function
    | {self.IsNotNumericFunction()}? atom
...
standard_function
    ...
    | json_function
    | {self.IsNotNumericFunction()}? other_function
    ;
