...
### #59〜#64 の修正後（2026-09-26）

黙って結果が変わる穴 6 件（#59〜#64）を直して、282 例を取り直した（`run_projects.py --from capture`）。

| | 最初の検証 | 修正後 |
|---|---:|---:|
| 一致 | 77 | **87** |
| 値が違う / 例外が出ない | 5 / 7 | **0** / 2（8-43 は Oracle 側の環境、9-26 は trigger の設計差） |
| 未対応で止まる / javac エラー / 別の例外 | 130 / 38 / 9 | 130 / 38 / 9 |
| 証拠つきの AUTO | 55（すべて一致） | **62**（すべて一致） |
| ルールだけの AUTO のうち一致しない例 | 57 | 47（黙って変わる例は 0） |

変わったのは 2-48、3-1、3-4、3-6、3-8、3-9、3-10、5-15、11-21、11-22 の 10 例で、ほかの例の結果は変わらない
（7-12 は取り直しの途中で #88 と同じ ScalarDB Cluster の INTERNAL エラーに当たり、namespace を作り直して前と同じ結果）。
同じ日に合成 corpus（両規約とも 92 シナリオで一致 89 / 相違 3、同等性 49/49）、tutorial（13/13）、
samples/oracle-samples（34 シナリオで一致 33、AUTO 16）も取り直し、数字は変わらなかった。
下の各節の数字と付録の表は、特に断らない限り最初の検証のもの（付録の表 1〜3 は修正後に作り直した）。
...
黙って結果が変わる穴を先に塞ぐのがよい。どれも 1 つの関数か 1 つの生成規則に閉じている（**6 件とも 2026-09-26 に直した。上の「修正後」**）:
...
| 種類 | Issue |
|---|---|
| 黙って結果が変わる（Priority High、2026-09-26 に直した） | #59 SUBTYPE の制約、#60 PLS_INTEGER / SIMPLE_INTEGER、#61 IN の 3 値論理、#62 CHAR の空白埋め、#63 ネストした表の =、#64 割り算の桁 |
