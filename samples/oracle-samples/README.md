...
第 1 部・第 2 部は 2026-09-24 の初回の記録で、そこで見つかった不具合を Issue にして直した結果を「Issue の修正後」の節と付録（修正後の数字）に置く。
...
| | 数 | 2026-09-24（初回） | 2026-09-25（Issue #30〜#42 の修正後） |
|---|---|---|---|
| SQL 文（6 ファイル） | 175 文 | OK 27 / WARN 54 / PLANNED 27 / ERROR 69 | OK 27 / WARN 54 / PLANNED 20 / ERROR 74。H2 で動かない 7 文が PLANNED から ERROR に、WITH FUNCTION が 1 文に |
...
## Issue の修正後（2026-09-25）

初回で「直していない穴」とした 13 件を GitLab の Issue #30〜#42 に登録し、順に直した（ブランチ `feat/oracle-samples-verification`）。
直したあと、同じ入力で SQL 変換・実 DB 比較・PL/SQL 生成・実 DB 比較を取り直した。付録の表は取り直したあとのもの。

| Issue | 何を直したか | 取り直した結果 |
|---|---|---|
| #30 | `WITH FUNCTION … SELECT … /` を 1 文として `WITH_PLSQL` で断る | 02 H-4 が 3 断片から 1 文の ERROR に |
| #31 | FULL OUTER JOIN / LATERAL・CROSS APPLY / SAMPLE は H2 で動かないので `RESIDUAL_H2`（ERROR） | B-3 / B-5 / H-2 が PLANNED → ERROR（実 DB の FAIL 3 → SKIP 3） |
| #32 | 再帰 WITH の `SEARCH DEPTH FIRST` も同じ（`VARCHAR2` は H2 の Oracle モードが読める） | G-3 が PLANNED → ERROR |
| #33 | `JSON_TABLE` を表として取得しない | 03 F-3 が PLANNED → ERROR（取得 SQL が壊れなくなった） |
| #34 | residual-runner が H2 の JSON 型を文字列で返す | 03 F-4 `JSON_OBJECT` が FAIL → **PASS** |
| #35 | 列並びを省いた INSERT の bind を表の定義順で列に帰属させ、DOUBLE 列へは double で bind | `b06_2_forall_save_exceptions` の DB-SQL-10016 は消えた（いまは `COMMIT` の TX-001 で止まる。決定待ち） |
| #36 | 全分岐が throw で終わる IF / CASE の後の文を出さない | `b04_4_4_ref_cursor` が部分集合に戻った（SYS_REFCURSOR そのものは未対応のまま） |
| #37 | 数値の FOR ループ（REVERSE 含む） | `b04_2_control_flow` が **一致**、AUTO |
| #38 | handler の `SQLERRM` / `FORMAT_ERROR_BACKTRACE` | `b04_6_2_user_exceptions` は `ROLLBACK`（TX-001）まで進む |
| #39 | 明示 cursor の OPEN / FETCH INTO %ROWTYPE / CLOSE と `%ROWCOUNT` | `b04_4_1_explicit_cursor` が **一致** |
| #40 | 判定によらず生成物が javac を通る（collection・record・TYPE コンストラクタ・package 変数は throw に、OUT 引数つき呼び出しは Result record から受ける、standalone の呼び先は Service を注入、関数の `RETURN;`、trigger check の引数） | 41 routine 全部がコンパイルでき、部分集合ではなく全ユニットで比べられるようになった（23 → 31 シナリオ） |
| #41 | シナリオで省いた引数を DEFAULT のリテラルで埋める | `annual_comp_null_comm` を `p_comm` 省略のまま比べられる |
| #42 | plsql_compare が Oracle DATE の 0 時と ScalarDB DATE を同じ値と見る | hire_date を DATE のままにしても比較が落ちない（本記録は TIMESTAMP のまま） |

あわせてハーネスを 1 つ直した: Oracle 側で package の状態が破棄される ORA-04068（シナリオごとに sequence を作り直すと package が無効になる）を 1 度だけ再試行する。

**サンプルの不備がもう 1 つ見つかった。** 全ユニットを Oracle に配備したところ、`emp_salary_audit_trg` が ORA-00984 でコンパイルできない
（`VALUES` 句の中の `CASE WHEN DELETING …`。trigger 述語は SQL の中では使えない）。この trigger が無効なまま employees を更新すると ORA-04098 になり、
9 シナリオが Oracle 側で落ちた。`split_sources.py` の `FIXES` で、`DELETING` を先に PL/SQL の変数へ取り出す最小の修正を当てている（意味は同じ、理由つき）。
...
**修正後の PL/SQL 実 DB 比較（31 シナリオ、一致 16 / 相違 15）**。相違 15 本の内訳（2026-09-25 に読み直して訂正。初出の記述は「値の差は無い」としていたが、DEFAULT 列の NULL は値の差である）:

| 相違の種類 | 本数 | シナリオ | 内容 |
|---|---|---|---|
| DDL の `DEFAULT` 句を INSERT に反映していない（生成器の穴） | 3 | `raise_salary_ok` `emp_api_give_raise_ok` `log_msg_ok` | `emp_audit.changed_by DEFAULT USER` / `changed_at DEFAULT SYSTIMESTAMP` が ScalarDB 側で NULL。IDENTITY 列は埋めるが DEFAULT 句は埋めていない。DOUBLE 変種の `6000` と `6000.0` は scale だけの差 |
| 決定の帰結（`callerBoundary`: 呼び出し側が戻さない ROLLBACK） | 4 | `b04_3` `b04_4_3` `b05_1` `b04_6_2` | Oracle が戻した trigger の監査行（`emp_audit` 2 行）や FK 違反にならなかった行（employees 999）が残る |
| 移行先に CHECK 制約が無い | 1 | `b06_2_forall_save_exceptions` | Oracle が CHECK で弾いた 4 行が `bulk_target` に入る |
| 呼び出し側の設計（別トランザクションの routine を呼ぶ） | 1 | `b05_4_call_log_msg` | 「境界を開くのは呼び出し側」として止まる |
| 生成器がまだ模していない構文 | 6 | `b05_3`（`INSERT … RETURNING` の `hire`）`b06_2_2`（FORALL … RETURNING）`b06_3`（動的 UPDATE の RETURNING）`b06_3_6`（DBMS_SQL）`b06_4`（オブジェクト型の `TABLE()`）`b06_5`（外部 package の名前付き引数） | 解析が断った所で throw。値の差は無い |
...
### 5 回目の対応（2026-09-25、Issue #47〜#50）

4 回目のあとに利用者が 2 つを決めた（AskUserQuestion）: #50 は「表ごとに決めて guard を生成」、実装の穴は「サンプルで効く順」（#48 → #47 → #49）。
...
### 6 回目の対応（2026-09-26、Issue #57 #58、SQL の実 DB 比較）

SQL の実 DB 比較に残っていた FAIL 5 本と CASE_ERROR 2 本は、毎回目で見て「非決定」「サンプルの側の事情」と判断していた。
ケースファイルの文の上に、比べ方と理由を宣言できるようにした（`difftest/case_notes.py`）。宣言は理由が無いと読まない。

| 宣言 | 意味 | この記録で使った文 |
|---|---|---|
| `-- @nondeterministic: count; reason=…` | 行数だけを比べる | A-1（`SYSDATE` / `USER`）、A-4（ORDER BY の無い `ROWNUM <= 5`） |
| `-- @nondeterministic: unordered; reason=…` | ORDER BY があっても集合として比べる | F-1 の `WITH TIES`（同順位の 2 人の並び） |
| `-- @nondeterministic: ignore=列; reason=…` | その列を両側から外して比べる（名前か 1 始まりの位置） | E-1'（`unordered; ignore=rn`。同順位の 2 人に `ROW_NUMBER` のどちらが付くか）、F-1 の `OFFSET`（`ignore=last_name`。7000 の社員がページの境目をまたぐ） |
| `-- @source-rejects: sample; reason=…` | 移行元が拒否する。移行元の不備 | E-1（別名 `share` が 26ai の予約語、ORA-00923） |
| `-- @source-rejects: harness; reason=…` | 移行元が拒否する。ハーネスの都合 | H-1（表を作った直後のフラッシュバック、ORA-01466） |

緩めた比べ方で一致したものは PASS（`DECLARED` として件数を出す）、それでも違えば FAIL。移行元が宣言どおり拒否したものは理由つきの SKIP（`SOURCE_REJECTS`）で、
宣言の無い拒否だけが CASE_ERROR に残る。H-1 は、表を作ってから 1 分以上あとに `--skip-setup` で流すと Oracle が答え、変換不可（SKIP）として数えられることも確かめた。

```text
PASS=24 FAIL=0 SKIP=16 CASE_ERROR=0 (of PASS, EMPTY=0 DECLARED=5; of SKIP, SOURCE_REJECTS=2)
```
...
| `emp_api.give_raise~1` | REDESIGN | SQL-004, STATE-001 | STATE-001: Package 変数はセッションに紐づく状態です。Singleton bean の field へ置くと意味が変わります |
| `emp_api.give_raise~2` | REDESIGN | CUR-002, SQL-004, STATE-001 | STATE-001: Package 変数はセッションに紐づく状態です。Singleton bean の field へ置くと意味が変わります |
| `emp_api.hire` | REDESIGN | EXC-001, STATE-001 | STATE-001: Package 変数はセッションに紐づく状態です。Singleton bean の field へ置くと意味が変わります |
