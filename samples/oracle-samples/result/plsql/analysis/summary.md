...
- **ERROR** `RMW` SET salary = salary + 1: expressions referencing columns are not allowed; do SELECT -> compute -> UPDATE with a literal inside one ScalarDB transaction（b05_4_call_log_msg.prc:4）
...
- **ERROR** `RETURNING` RETURNING is not supported（emp_api.pkb:19-23）
- **ERROR** `RMW` SET salary = salary * (1 + :p_pct / 100): expressions referencing columns are not allowed; do SELECT -> compute -> UPDATE with a literal inside one ScalarDB transaction（emp_api.pkb:33）
...
| `b05_3_call_emp_api` | procedure | 1 | 9 | — |
| `b05_4_call_log_msg` | procedure | 1 | 9 | — |
